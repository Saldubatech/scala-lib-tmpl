...
class QuillPlatform(val quill: Quill.Postgres[SnakeCase]) extends Platform:
  ...
  abstract class BaseQuillRepo[E <: CASE_LIKE: Typeable](val maxRecords: Int = 10000) extends Repo[E, QPlatformIO]:
    ...
    implicit def orderRequirement(using ord: Ordering[E]): Sort[E] =
      new Sort[E]() {
...
trait QuillRepo[T]:
  ...
  object RepoHelper {
    ...
    inline def findByQuery: Quoted[Query[T]] => IO[SQLException, List[T]] = (q: Quoted[Query[T]]) => platform.quill.run(q)
    ...
    inline def finderBy[PROPERTY: Encoder](inline bq: EntityQuery[T], inline selector: T => PROPERTY)
        : PROPERTY => Quoted[Query[T]] = (p: PROPERTY) => bq.filter(host => selector(host) == lift(p)).take(1)

    inline def finderByKeys(inline bq: EntityQuery[T], inline values: Map[String, Any]): Query[T] = bq.filterByKeys(values)
  ...
  def countAll: PersistenceIO[Long] = allRecordsCounter.mapError(exc => PersistenceError("SQL Error", Some(exc)))
