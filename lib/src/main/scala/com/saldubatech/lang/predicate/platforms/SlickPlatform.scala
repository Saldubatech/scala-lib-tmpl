...
import com.saldubatech.infrastructure.storage.{InsertionError, PersistenceError, RepositoryError}
...
class SlickPlatform(val dbP: DatabaseProvider) extends Platform:
  ...
  abstract class BaseSlickRepo[V](val maxRecords: Int = 10000) extends Repo[V, DBIO] with SlickRepoProfile[V]:
    ...
    override def find[P <: Predicate[STORAGE]](p: P)(using prj: platform.REQUIRES[STORAGE, P]): DBIO[Seq[V]] =
...
abstract class SlickRepoZioService[E: ZTag](platform: SlickPlatform) extends LogEnabled:
  ...
  private def mapFromDBIO[RS](action: DBIO[RS]): REPO_IO[RS] =
    ZIO.fromDBIO[RS](action).provideEnvironment(ZEnvironment(platform.dbP)).mapError {
  ...
  private def mapFromDBIOMap[DBRS, RS](action: DBIO[DBRS])(pf: DBRS => REPO_IO[RS]): REPO_IO[RS] =
    ZIO
      .fromDBIO[DBRS](action)
      .provideEnvironment(ZEnvironment(platform.dbP))
      .flatMap(zr => pf(zr))
      .mapError {
        case pe: PersistenceError => pe
        case other: Throwable     => RepositoryError.fromThrowable(other)
  ...
  def find[PRED <: Predicate[repo.STORAGE]](p: PRED)(using repo.platform.REQUIRES[repo.STORAGE, PRED]): REPO_IO[Seq[E]] =
    mapFromDBIO[Seq[E]](repo.find(p))
