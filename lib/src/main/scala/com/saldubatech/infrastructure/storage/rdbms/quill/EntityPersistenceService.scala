...
import com.saldubatech.infrastructure.storage.{Payload, RepositoryError, TimeCoordinates}
...
trait EntityPersistenceService[P <: Payload, E <: EntityType[P]](using val entity: E):
  ...
  abstract protected class BaseRepo(val quill: Quill.Postgres[Literal]) extends EntityRepo:

    import quill.*

    protected val adder: (P, Id) => EIO[Id]

    protected inline def addG(inline qSchema: Quoted[EntityQuery[Record]]): (P, Id) => ZIO[Any, RepositoryError, Id] =
      (d: P, overrideRId: Id) =>
        run {
          quote {
            qSchema
        ...
        }.either.resurrect.refineOrDie { case e: NullPointerException =>
    ...
    protected inline def deleteG(inline qSchema: Quoted[EntityQuery[Record]]): Id => EIO[Long] =
      (id: Id) =>
        run {
          quote {
    ...
    protected inline def getAllG(inline qSchema: Quoted[EntityQuery[Record]])
        : () => ZIO[Any, RepositoryError, List[entity.Record]] =
      () =>
        run {
          quote {
            qSchema
    ...
    protected inline def getByIdG(inline qSchema: Quoted[EntityQuery[Record]]): Id => EIO[Option[Record]] =
      id =>
        ...
        }.map(_.headOption).refineOrDie { case e: SQLException =>
          RepositoryError.fromThrowable(e)
        }
    ...
    protected inline def updateG(inline qSchema: Quoted[EntityQuery[Record]]): (Id, P) => EIO[Option[Unit]] =
      (rId: Id, data: P) =>
        ...
        }.map(n => if n > 0 then Some(()) else None).refineOrDie { case e: SQLException =>
          RepositoryError.fromThrowable(e)
        }
