...
class SampleZioService(using ExecutionContext)(val platform: SlickPlatform) extends SlickRepoZioService[Sample](platform):
  ...
  override val repo: Repo[Sample, DBIO] = new platform.BaseSlickRepo[Sample](10000) {
    ...
    lazy val tblTag     = summon[Typeable[SampleTable]]
    lazy val tableQuery = _tq
  } // platform.repoFor[Sample, SampleTable](_tq, 10000)

object SampleZioService:

  def zioLayer(using ExecutionContext): URLayer[SlickPlatform, SampleZioService] =
    ZLayer {
      for {
